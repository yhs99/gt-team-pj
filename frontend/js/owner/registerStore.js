new Vue({
  ...
  updateFileCount() {
    ...
    toggleFacility(facility) {
      const index = this.facilityDTO.findIndex(f => f.facilityCode === facility.facilityCode);
      if (index === -1) {
    ...
    facilityIsSelected(facility) {
      return this.facilityDTO.some(f => f.facilityCode === facility.facilityCode);
    },
    // 카테고리 선택 토글 메서드
    toggleCategory(category) {
      const index = this.storeCategoryDTO.findIndex(c => c.categoryCodeId === category.categoryId);
      if (index === -1) {
        this.storeCategoryDTO.push({ categoryCodeId: category.categoryId, storeCategoryName: category.categoryName });
      } else {
        this.storeCategoryDTO.splice(index, 1);
      }
    },
    isSelected(category) {
      return this.storeCategoryDTO.some(c => c.categoryCodeId === category.categoryId);
    },
    ...
    sample6_execDaumPostcode() {
      new daum.Postcode({
        oncomplete: (data) => {
          ...
          this.detailAddress = document.getElementById("sample6_detailAddress").value;
