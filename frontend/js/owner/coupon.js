new Vue({
  ...
  computed: {
    formattedDiscount() {
      return this.newCoupon.discount > 100
        ? `${this.newCoupon.discount} 원`
  ...
  methods: {
    ...
    getAllCoupons() {
      axios
        .get("/api/coupon")
        .then((response) => {
          this.coupons = Array.isArray(response.data.data)
            ? response.data.data
            : [];
    ...
    startEdit(couponId) {
      const couponToEdit = this.formattedCoupons.find(
    ...
    updateCoupon() {
      ...
      // 업데이트 요청
      axios
        .put(`/api/coupon/${this.selectedCouponId}`, formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
