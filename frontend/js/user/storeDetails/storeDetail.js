...
new Vue({
    ...
    methods: {
        ...
        async setBookmark(){
           try{
               const response = await axios.post(`/api/bookmark/30`);
        ...
        async deleteBookmark(){
            try{
                const response = await axios.delete(`/api/bookmark/30`);
                console.log(response);
        ...
        async getBookmarkInfo(){
            try{
                ...
                console.error('북마크 가져오기 에러(로그인)',error);
            }
        ...
        checkIfBookmarked(){
            ...
            for(bookmark of this.currentBookmark){
                if(bookmark.bookmarkDto.storeId == 30){
                    this.isBookmarked = true;
