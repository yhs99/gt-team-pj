new Vue({
    ...
    methods: {
        async fetchRestaurantData() {
            try {
                const response = await axios.get(`/api/stores/store/30`);
        ...
        async fetchMenuData() {
            try {
                const response = await axios.get(`/api/stores/menu/30`);
        ...
        async fetchReviewData() {
            try {
                const response = await axios.get(`/api/review/store/146`);
        ...
        async fetchRecommendData() {
            try {
                const response = await axios.get(`/api/stores/storeInfo/146`);
        ...
        makeACalendar() {
            ...
            const calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                dateClick:(info) => {
                    ...
                    // 클릭한 날짜의 배경색 변경
                    info.dayEl.style.backgroundColor = '#c2eae7';
                    // 클릭한 날짜를 저장
        ...
        async fetchCouponData() {
            try {
                const response = await axios.get(`/api/stores/coupon/146`);
        ...
        async fetchReserveSlots(date) {
            console.log(date);
            try{
                const response = await axios.get(`/api/stores/reserveSlots/30/${date}`)
        ...
        logCheckedMenus() {
            ...
            if (checkedMenus.length > 0) {
                ...
                console.log('체크된 메뉴가 없습니다.');
            }
            
            if (this.selectedSlot !== null) {
                const selectedSlotDatetime = this.reserveSlots[this.selectedSlot]?.slotDatetime;
                console.log('Selected Slot Datetime:', selectedSlotDatetime);
                const queryString = `?reserveTime=${selectedSlotDatetime}`;
                const targetUrl = `http://localhost/view/user/cart${queryString}`;
    
                // window.location.href = targetUrl;
            } else {
                console.log('슬롯이 선택되지 않았습니다.');
            }
        },
